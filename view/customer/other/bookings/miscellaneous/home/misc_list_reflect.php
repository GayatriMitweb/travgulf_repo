...
<table class="table table-bordered bg_white cust_table" id="tbl_miscellaneous_list" style="margin:20px 0 !important">
	...
	<tbody>
		...
		while($row_miscellaneous = mysql_fetch_assoc($sq_miscellaneous)){
			...
			$pass_count = mysql_num_rows(mysql_query("select * from  miscellaneous_master_entries where misc_id='$row_miscellaneous[misc_id]'"));
			$cancel_count = mysql_num_rows(mysql_query("select * from  miscellaneous_master_entries where misc_id='$row_miscellaneous[misc_id]' and status='Cancel'"));
			$bg="";
