...
while($row_taxes = mysql_fetch_assoc($sq_taxes)){
	...
	$sq_other = mysql_fetch_assoc(mysql_query("select * from other_charges_master where entry_id='$row_taxes[rule_for]'"));
	...
	$temp_arr = array("data" =>array(
		...
		<button class="btn btn-warning btn-sm" onclick="copy_rule('.$row_taxes['rule_id'] .')" data-toggle="tooltip" title="Copy Other Rule"><i class="fa fa-files-o"></i></button>
		'), "bg" => $bg
	);
