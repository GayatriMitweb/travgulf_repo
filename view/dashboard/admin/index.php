				                  ...
				                  <div class="col-md-12 no-pad table_verflow"> 
				                    <div class="table-responsive">
				                      <table class="table table-hover" style="margin: 0 !important;border: 0;">
										...
										<!-- Hotel Booking -->
										<?php
										$query1 = "select * from  hotel_booking_entries where status!='Cancel' and check_in >= '$today'
										group by booking_id";
				          				...
				          				while($row_query=mysql_fetch_assoc($sq_query)){
											...
											?>
												...
												<td class="text-center"><h6 style="width: 90px;height: 30px;border-radius: 20px;font-size: 12px;line-height: 21px;text-align: center;background:<?= $bg_color ?>;padding:5px;color:<?= $text_color?>"><?= $status ?></h6></td>
												<td class="text-center"><button class="btn btn-info btn-sm" onclick="whatsapp_wishes('<?= $contact_no ?>','<?= $sq_cust['first_name']?>')" title="" data-toggle="tooltip" data-original-title="What'sApp wishes to customer"><i class="fa fa-whatsapp"></i></button></td>
											</tr>
											<?php } } ?>
										<!-- Flight Booking -->
										<?php
										...
										<!-- Train Booking -->
										<?php
										$sq_query_train = mysql_query("select * from  train_ticket_master_trip_entries where travel_datetime >= '$today' group by train_ticket_id");
										while($row_query1=mysql_fetch_assoc($sq_query_train)){
