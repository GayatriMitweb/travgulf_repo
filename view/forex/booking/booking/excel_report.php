...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('G'.$row_count, "Forex Amount")
...
while($row_booking = mysql_fetch_assoc($sq_booking)){
    ...
    $emp_name = ($row_booking['emp_id'] != 0) ? $sq_emp['first_name'].' '.$sq_emp['last_name'] : 'Admin';
			...
			$sq_amount = mysql_fetch_assoc(mysql_query("SELECT sum(`credit_charges`) as sumc from forex_booking_payment_master where booking_id='$row_booking[booking_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));    
            $credit_card_charges = $sq_amount['sumc'];
...
->setCellValue('H'.$row_count, "Total: ".$booking_total)
