...
while($row_customer = mysql_fetch_assoc($sq_customer)){
    $sq_credit = mysql_fetch_assoc(mysql_query("select * from b2b_creditlimit_master where register_id='$row_customer[register_id]' order by entry_id desc"));
    $sq_currency = mysql_fetch_assoc(mysql_query("select currency_code from currency_name_master where id='$row_customer[currency]'"));
