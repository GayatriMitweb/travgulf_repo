...
$query = "select * from insuarance_vendor where 1 ";
if($active_flag!=""){
	...
	<tbody>
		...
		while($row_vendor = mysql_fetch_assoc($sq_vendor)){
			...
			<tr class="<?= $bg ?>">
				...
				<td>
					<button class="btn btn-info btn-sm" onclick="update_modal(<?= $row_vendor['vendor_id'] ?>)" title="Edit"><i class="fa fa-pencil-square-o"></i></button>
				</td>
			</tr>
			<?php
		}
