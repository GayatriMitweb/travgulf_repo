...
$sq_passport = mysql_fetch_assoc(mysql_query("select * from passport_master where passport_id='$sq_payment_info[passport_id]'"));
...
<div class="modal fade" id="passport_payment_update_modal" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="frm_passport_payment_update">
         ...
         <div class="row">
          <div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
              <select name="customer_id1" id="customer_id1" title="Customer Name" style="width:100%" onchange="passport_id_dropdown_load('customer_id1', 'passport_id1');" disabled>
                <?php 
          ...
          <div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
            <select name="passport_id1" id="passport_id1" style="width:100%" title="Booking ID" disabled>              
			         <option value="<?= $sq_passport['passport_id'] ?>"><?= 'PS/'.$year.'/'.$sq_passport['passport_id'] ?></option>
              <?php
              $sq_passport = mysql_query("select * from passport_master where customer_id='$sq_passport[customer_id]'");
              while($row_passport = mysql_fetch_assoc($sq_passport)){
                ?>
                <option value="<?= $row_passport['passport_id'] ?>"><?= 'PS/'.$year.'/'.$row_passport['passport_id'] ?></option>
                <?php
              }
              ?>
            </select>
          ...
          <div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10_xs">
            <input type="text" id="transaction_id1" name="transaction_id1" class="form-control" placeholder="Cheque No/ID" title="Cheque No/ID" value="<?= $sq_payment_info['transaction_id'] ?>" <?= $enable ?>>
          </div>
        ...
        <div class="row text-center mg_tp_20">
            <div class="col-xs-12">
              <button class="btn btn-sm btn-success" id="pass_p_update"><i class="fa fa-floppy-o"></i>&nbsp;&nbsp;Update</button>
            </div>
        </div>
...
<script>
$('#customer_id1, #passport_id1').select2();
$('#payment_date').datetimepicker({ timepicker:false, format:'d-m-Y' });

$('#passport_payment_update_modal').modal('show');  
$(function(){

$('#frm_passport_payment_update').validate({
  rules:{
    passport_id1 : { required : true },
  ...
  submitHandler:function(form){
    ...
    $('#pass_p_update').button('loading');
    $.ajax({
      type: 'post',
      url: base_url+'controller/visa_passport_ticket/passport/passport_master_payment_update.php',
      data:{ payment_id : payment_id, passport_id : passport_id, payment_date : payment_date, payment_amount : payment_amount, payment_mode : payment_mode, bank_name : bank_name, transaction_id : transaction_id, bank_id : bank_id,payment_old_value : payment_old_value,credit_charges:credit_charges,credit_card_details:credit_card_details,credit_charges_old:credit_charges_old },
      success: function(result){
        var msg = result.split('-');
        if(msg[0]=='error'){
          msg_alert(result);
          $('#pass_p_update').button('reset');
        }
        else{
          msg_alert(result);
          reset_form('frm_passport_payment_update');
          $('#pass_p_update').button('reset');
          $('#passport_payment_update_modal').modal('hide');  
