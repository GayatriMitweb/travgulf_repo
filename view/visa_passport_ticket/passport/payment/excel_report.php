...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C2', 'Passport Payment')
...
while($row_passport_payment = mysql_fetch_assoc($sq_passport_payment)){
    ...
    ->setCellValue('C'.$row_count, get_passport_booking_id($row_passport_payment['passport_id'],$year))
    ->setCellValue('D'.$row_count, $customer_name)
    ->setCellValue('E'.$row_count, date('d-m-Y', strtotime($row_passport_payment['payment_date'])))
    ->setCellValue('F'.$row_count, $row_passport_payment['payment_mode'])
    ->setCellValue('G'.$row_count, number_format($row_passport_payment['payment_amount']+$row_passport_payment['credit_charges'],2));
...
header('Content-Disposition: attachment;filename="PassportPayment('.date('d-m-Y H:i:s').').xls"');
