	...
	<div class="col-md-10  col-md-offset-1 col-sm-12 col-xs-12">
		<div class="widget_parent-bg-img bg-img-red">
			<?php     
				...
				$sq_payment_info = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from visa_payment_master where visa_id='$visa_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
				$sq_refund_info = mysql_fetch_assoc(mysql_query("select sum(refund_amount) as sum from visa_refund_master where visa_id='$visa_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
				
				if($sq_visa_info['visa_issue_amount'] == ''){ $visa_amount = 0; }
				else{ $visa_amount = $sq_visa_info['visa_issue_amount']; }
...
if($sq_visa_info['service_tax_subtotal'] !== 0.00 && ($sq_hosq_visa_infotel['service_tax_subtotal']) !== ''){
...
if($bsmValues[0]->service != ''){   //inclusive service charge
  ...
  $tax_show = $name.' '.$currency_code." ".number_format($sq_visa_info['service_tax_subtotal'],2);
...
if($bsmValues[0]->basic != ''){ //inclusive markup
  ...
  $tax_show = '';
	            ...
	            begin_widget();
	                ...
	                $content_arr = array( number_format($visa_amount,2) ,number_format($service_charge,2), number_format($service_tax_amount + $markupservice_tax_amount,2),number_format($sq_visa_info['markup'],2),number_format($sq_visa_info['roundoff'],2), number_format($sq_visa_info['visa_total_cost'],2) ,number_format($sq_payment_info['sum'],2) );
	                $percent = ($sq_payment_info['sum']/$sq_visa_info['visa_total_cost'])*100;
...
<div class="row">
	...
	<input type="checkbox" id="check_all" name="check_all" onClick="select_all_check(this.id,'traveler_names')">&nbsp;&nbsp;&nbsp;<span style="text-transform: initial;">Check All</span>
		<table class="table table-hover table-bordered mg_bt_0" style="margin: 0 !important;">
			...
			$count = 0;
			$disabled_count= 0;
			$sq_visa_entries = mysql_query("select * from visa_master_entries where visa_id='$visa_id'");
...
<hr>
<?php
$sq_cancel_count = mysql_num_rows(mysql_query("select * from visa_master_entries where visa_id='$visa_id' and status='Cancel'"));
if($sq_cancel_count>0){
	$sq_visa_info = mysql_fetch_assoc(mysql_query("select * from visa_master where visa_id='$visa_id'"));
?>
	<form id="frm_refund">

		<div class="row text-center mg_bt_150">
                ...
                callback: function(data1){
                    if(data1=="yes"){
                        ...
                        $.ajax({
                          ...
                          data:{ visa_id : visa_id,cancel_amount : cancel_amount, total_refund_amount : total_refund_amount },
                          success:function(result){
                            msg_alert(result);
                            visa_entries_reflect();
