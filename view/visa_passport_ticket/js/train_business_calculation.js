function get_auto_values(
	...
	$('#markup_show').html('&nbsp;');
	$('#markup_show1').html('&nbsp;');
	const rules = get_other_rules('Train', booking_date);
	console.log(rules);
...
function get_tax_rules_on_conditions(final_taxes_rules, basic_amount, payment_mode, type) {
	let applied_rules = [];
	if (type === 'save') {
		var hotel_table = 'tbl_dynamic_train_ticket_master';
						...
						condition.forEach((cond) => {
							...
							switch (condition) {
								...
								case '5':
									...
									break;
								case '8':
									if (value == 'Train' || value == 'All') flag = true;
									break;
								case '3':
									switch (for1) {
										case '!=':
