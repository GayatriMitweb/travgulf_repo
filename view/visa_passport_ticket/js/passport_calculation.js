function get_auto_values (booking_date, sub_total, payment_mode, service_charge, type, charges_flag, amount_type,change=false) {
	...
	const rules = get_other_rules('Passport', booking_date);
	...
	if (charges_flag === 'true') {
		...
		if (parseInt(rules_array.length) === 0) {
			...
			var service_charge1 = calculate_charges(rules_array, type, basic_amount);
			...
			$('#passport_sc').val(rules_array[0].ledger_id);
		}
	...
	if (new_taxes_rules.length > 0) {
		new_taxes_rules &&
			new_taxes_rules.map((rule) => {
				...
				console.log(charge_amount+' ss '+target_amount);
				...
				tax_amount =tax_amount !== '' || typeof tax_amount !== NaN || tax_amount !== undefined ? parseFloat(tax_amount).toFixed(2) :
				...
				if (calculation_mode !== '"Exclusive"') {
					if (tax_for === 'service_charge') {
						...
						$('#passport_taxes').val(ledger_posting);
					}
				}
				else {
					if (tax_for === 'service_charge') {
						...
						condition.forEach((cond) => {
							...
							switch (condition) {
								...
								case '5':
									...
									break;
								case '8':
									if (value == 'Passport' || value == 'All') flag = true;
									break;
								case '3':
									switch (for1) {
										case '!=':
...
function calculate_charges (rules_array, type, basic_amount) {
	if (rules_array.length) {
		...
		if (rules_array[0].target_amount != '') {
			if (rules_array[0].target_amount === 'Basic') var target_amount = basic_amount;
				...
				var target_amount = parseFloat(basic_amount);
		}
		else var target_amount = 0;

		if (type === 'save') {
			var pass_table = 'tbl_dynamic_passport';
		}
		else {
			var pass_table = 'tbl_dynamic_passport_update';
		}

		var table = document.getElementById(pass_table);
		var rowCount = table.rows.length;
		var pax_count = 0;

		for (var i = 0; i < rowCount; i++) {
			var row = table.rows[i];
			if (row.cells[0].childNodes[0].checked && (row.cells[7].childNodes[0].value != 'Infant' && row.cells[7].childNodes[0].value != '')) ++pax_count;
		}
		switch (apply_on) {
			case '1':
				//Per pax
