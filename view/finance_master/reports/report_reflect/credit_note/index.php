...
<table id="credit" class="table table-hover" style="margin: 20px 0 !important;">         
</table>
</div></div></div>
</div>
...
function report_reflect()
{
	...
	var role = $('#role').val();
	$.post('report_reflect/credit_note/report_reflect.php',{to_date : to_date ,branch_status : branch_status, role : role,branch_admin_id : branch_admin_id }, function(data){
		pagination_load(data, column, true, false, 20, 'credit');
		$('.loader').remove();
	});
}
report_reflect();
function excel_report()
{
	...
	var branch_admin_id = $('#branch_admin_id').val();
	var role = $('#role').val();
	window.location = 'report_reflect/credit_note/excel_report.php?to_date='+to_date+'&branch_status='+branch_status+'&branch_admin_id='+branch_admin_id+'&role='+role;
}
</script>
