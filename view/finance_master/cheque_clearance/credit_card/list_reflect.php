...
$query = "select * from bank_cash_book_master where 1 and payment_mode='Credit Card' ";
if($status!="All"){
...
<table class="table table-bordered" id="tbl_list1" style="margin: 20px 0 !important;">
 	<thead>
 	...
 	<tbody>
		...
		<tr class="<?= $bg ?>">
			...
			<td>
				<?php if($row_transaction['clearance_status']!="Cleared" && $row_transaction['payment_mode'] == 'Credit Card'){ ?>
				<button class="btn btn-info btn-sm" onclick="clearance_status_change1(this, <?= $row_transaction['register_id'] ?>, 'Cleared', 'status_date<?= $count ?>','<?= $row_transaction['module_name'] ?>',<?= $row_transaction['module_entry_id'] ?>,'',<?= $row_transaction['payment_amount']?>,'<?= $row_transaction['particular']?>','card')" title="Clear"><i class="fa fa-check-square-o"></i></button>
				<?php } ?>
			</td>
			<td>
				<?php if($row_transaction['clearance_status']!="Cancelled" && $row_transaction['payment_mode'] == 'Credit Card'){ ?>
				<button class="btn btn-danger btn-sm" onclick="clearance_status_change1(this, <?= $row_transaction['register_id'] ?>, 'Cancelled', 'status_date<?= $count ?>','<?= $row_transaction['module_name'] ?>',<?= $row_transaction['module_entry_id'] ?>,'',<?= $row_transaction['payment_amount']?>,'<?= $row_transaction['particular']?>','card')" title="Cancel cheque"><i class="fa fa-times"></i></button>
				<?php } ?>
			</td>
		</tr>
		<script>
...
function clearance_status_change1(ele, register_id, status, status_date_id,module_name,module_entry_id,transaction_id,payment_amount,particular,type){
