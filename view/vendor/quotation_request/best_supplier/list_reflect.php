...
while($row_req = mysql_fetch_assoc($sq_req)){
	...
	$temp_arr = array( "data" => array(
		...
		$row_req['supplier_id'],
		$row_req['sum'],
		'<button class="btn btn-info btn-sm" onclick="view_modal('. $row_req['supplier_id'] .',\''. $row_req['quotation_for'] .'\',\''. $sq_request['enquiry_id'] .'\')" data-toggle="tooltip" title="View"><i class="fa fa-eye"></i></button>'
	), "bg" =>$bg);
