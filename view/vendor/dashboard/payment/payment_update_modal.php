...
$sq_est = mysql_fetch_assoc(mysql_query("select sum(net_total) as net_total from vendor_estimate where vendor_type='$sq_payment[vendor_type]' and vendor_type_id='$sq_payment[vendor_type_id]' and estimate_type='$sq_payment[estimate_type]' and estimate_type_id='$sq_payment[estimate_type_id]'"));

$sq_paid = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as payment_amount from vendor_payment_master where vendor_type='$sq_payment[vendor_type]' and vendor_type_id='$sq_payment[vendor_type_id]' and estimate_type='$sq_payment[estimate_type]' and estimate_type_id='$sq_payment[estimate_type_id]'"));
  ...
  $('#frm_vendor_payment_update').validate({
      ...
      submitHandler:function(form){
			  ...
			  if(payment_amount > payment_old_value){
				 ...
				 if(parseFloat(balance_paying) > parseFloat(balance_amount)) { error_msg_alert('Payment should not be more than purchase amount!'); return false; }
			  }              
