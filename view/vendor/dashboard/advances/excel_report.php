...
$count = 0;
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('H'.$row_count, $row_payment['bank_name'])
        ->setCellValue('I'.$row_count, $row_payment['transaction_id']);
	...
	$objPHPExcel->getActiveSheet()->getStyle('B'.$row_count.':I'.$row_count)->applyFromArray($borderArray);    
        ...
        ->setCellValue('F'.$row_count, "Total Amount : ".number_format($total_payment, 2))
        ...
        ->setCellValue('I'.$row_count, "Total Paid : ".number_format(($total_payment - $sq_pending_amount - $sq_cancel_amount), 2));
...
header('Content-Disposition: attachment;filename="PrePurchase Advances('.date('d-m-Y H:i:s').').xls"');
