...
<form id="section_package">
    ...
    <table id="tbl_dest_packages" name="tbl_dest_packages" class="table border_0 table-hover">
        ...
        <tr>
            ...
            <td><select name="dest_name-1" id="dest_name-1" onchange="package_dynamic_reflect(this.id)" style="width:250px;" class="form-control app_select2">
            ...
            <td><select id="package-1" name="package-1" title="Select Package" class="form-control" onchange="get_package_image(this.id)" style="width:250px;">
                    <option value="">*Select Package</option>
                </select></td>
            <td><select id="image-1" name="image-1" title="Select Image" class="form-control app_select2" style="width:300px;">
                </select></td>
            <script>
            $('#dest_name-1').select2();
            </script>
        ...
        for($i=0;$i<sizeof($popular_dest);$i++){
            ...
            $sq_package = mysql_fetch_assoc(mysql_query("select package_id,package_name,dest_id from custom_package_master where package_id='$package_id'"));
        ...
        <tr>
            ...
            <td><select id="package-1<?=$i?>_u" name="package-1<?=$i?>_u" title="Select Package" class="form-control" style="width:250px;" onchange="get_package_image(this.id)">
                    <?php if($package_id!='0'){?>
                        <option value="<?= $sq_package['package_id'] ?>"><?= $sq_package['package_name'] ?></option>
                    <?php } ?>
                    <option value="">*Select Package</option>
                </select></td>
            <td><select id="image-1<?=$i?>_u" name="image-1<?=$i?>_u" title="Select Image" class="form-control app_select2" style="width:300px;">
