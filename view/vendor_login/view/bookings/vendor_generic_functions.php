...
function get_estimate_type_name($estimate_type, $estimate_type_id){
	...
	if($estimate_type=="Package Tour"){
		...
		$estimate_type_val = get_package_booking_id($sq_booking['booking_id'])." : ".$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Car Rental"){
		...
		$sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_booking[customer_id]'"));			
		$estimate_type_val = get_car_rental_booking_id($sq_booking['booking_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Visa Booking"){
		...
		$sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_visa[customer_id]'"));
   		$estimate_type_val = get_visa_booking_id($sq_visa['visa_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Passport Booking"){
		...
		$sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_passport[customer_id]'"));
   		$estimate_type_val = get_passport_booking_id($sq_passport['passport_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Ticket Booking"){
		...
		$sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_ticket[customer_id]'"));
   		$estimate_type_val = get_ticket_booking_id($sq_ticket['ticket_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Train Ticket Booking"){
		...
		$sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_ticket[customer_id]'"));
   		$estimate_type_val = get_train_ticket_booking_id($sq_ticket['train_ticket_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Hotel Booking"){
		...
		$sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_hotel_booking[customer_id]'"));
   		$estimate_type_val = get_hotel_booking_id($sq_hotel_booking['booking_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Bus Booking"){
        ...
        $sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_booking[customer_id]'"));
        $estimate_type_val = get_bus_booking_id($sq_booking['booking_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Forex Booking"){
        ...
        $sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_booking[customer_id]'"));
        $estimate_type_val = get_forex_booking_id($sq_booking['booking_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Miscellaneous Booking"){
		$sq_booking = mysql_fetch_assoc(mysql_query("select * from miscellaneous_booking_master where booking_id='$estimate_type_id'"));
        $sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_booking[customer_id]'"));
        $estimate_type_val = get_miscellaneous_booking_id($sq_booking['booking_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
	if($estimate_type=="Excursion Booking"){
		...
		$sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_exc[customer_id]'"));
   		$estimate_type_val = get_exc_booking_id($sq_exc['exc_id']).' : '.$sq_customer['first_name'].' '.$sq_customer['last_name'];
	}
...
function get_opening_bal($vendor_type, $vendor_type_id)
{
	...
	$sq_hotel = mysql_fetch_assoc(mysql_query("select * from ledger_master where customer_id='$vendor_type_id' and user_type='$vendor_type'"));
	$values[0] = $sq_hotel['balance'];
	$values[1] = $sq_hotel['dr_cr'];
