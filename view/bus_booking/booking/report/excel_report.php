...
$objPHPExcel->getActiveSheet()->getStyle('B'.$row_count.':I'.$row_count)->applyFromArray($borderArray);    
	...
	$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D'.$row_count, $sq_customer['first_name'].' '.$sq_customer['middle_name'].' '.$sq_customer['last_name'])
...
header('Content-Disposition: attachment;filename="Bus Report('.date('d-m-Y H:i:s').').xls"');
