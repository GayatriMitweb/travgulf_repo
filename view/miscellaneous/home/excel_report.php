...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('F'.$row_count, "Total Guest")
...
while($row_visa = mysql_fetch_assoc($sq_visa)){
    ...
    //Get Total visa cost
    $sq_visa_total_cost=mysql_fetch_array(mysql_query("select * from miscellaneous_master where misc_id='$row_visa[misc_id]'"));
	...
	$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('G'.$row_count, number_format($visa_total_amount+$credit_card_charges,2))
        ->setCellValue('H'.$row_count, number_format($cancel_amount,2))
        ->setCellValue('I'.$row_count, number_format($total_visa_amount,2))
...
header('Content-Disposition: attachment;filename="Miscellneous Booking('.date('d-m-Y H:i:s').').xls"');
