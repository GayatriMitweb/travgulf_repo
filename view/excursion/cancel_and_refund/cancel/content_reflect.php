	...
	<div class="col-md-10 col-md-offset-1 col-sm-8 col-sm-offset-2 col-xs-12 mg_bt_20_xs">
		<div class="widget_parent-bg-img bg-img-red">
			<?php     
				...
				$sq_payment_info = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from exc_payment_master where exc_id='$exc_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
				$sq_refund_info = mysql_fetch_assoc(mysql_query("select sum(refund_amount) as sum from exc_refund_master where exc_id='$exc_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
...
if($bsmValues[0]->basic != ''){ //inclusive markup
 ...
 $newBasic = $basic_cost1 + $service_tax_amount + $sq_exc_info['markup'] + $markupservice_tax_amount;
  $tax_show = '';
	            ...
	            begin_widget();
	                $title_arr = array("Booking","Serv. Charge", " Tax","Markup" ,"Roundoff","Total Amount","Paid Amount");
	                $content_arr = array( number_format($exc_amount,2) ,number_format($service_charge,2), number_format($service_tax_amount + $markupservice_tax_amount,2),number_format($sq_exc_info['markup'],2),number_format($sq_exc_info['roundoff'],2), number_format($sq_exc_info['exc_total_cost'],2),number_format($sq_payment_info['sum'],2));
	                $percent = ($sq_payment_info['sum']/$sq_exc_info['exc_total_cost'])*100;
	...
	<input type="checkbox" id="check_all" name="check_all" onClick="select_all_check(this.id,'traveler_names')">&nbsp;&nbsp;&nbsp;<span style="text-transform: initial;">Check All</span>
		<table class="table table-hover table-bordered mg_bt_0" style="margin: 0 !important;">
			...
			$count = 0;
			$disabled_count =0;
			$sq_exc_entries = mysql_query("select * from excursion_master_entries where exc_id='$exc_id'");
			while($row_entry = mysql_fetch_assoc($sq_exc_entries)){
...
<hr>
<?php 
$sq_cancel_count = mysql_num_rows(mysql_query("select * from excursion_master_entries where exc_id='$exc_id' and status='Cancel'"));
...
<form id="frm_refund" class="mg_bt_150">
	...
	<div class="row text-center">
		...
		<div class="col-md-3 col-sm-6 col-xs-12 mg_bt_10_xs">
			<input type="text" name="total_refund_amount" id="total_refund_amount" class="amount_feild_highlight text-right" placeholder="Total Refund" title="Total Refund" readonly value="<?= $sq_exc_info['total_refund_amount'] ?>">
		</div>
	</div>
	<?php if($sq_exc_info['cancel_amount'] == "0.00"){ ?>
	<div class="row mg_tp_20">
...
function cancel_booking(){
	...
	if(len!=pass_count){
		error_msg_alert('Please select all excursions for cancellations.');
	}
                ...
                callback: function(data1){
                    if(data1=="yes"){
                        ...
                        $.ajax({
                          ...
                          data:{ exc_id : exc_id,cancel_amount : cancel_amount, total_refund_amount : total_refund_amount  },
                          success:function(result){
                            msg_alert(result);
                            exc_entries_reflect();
