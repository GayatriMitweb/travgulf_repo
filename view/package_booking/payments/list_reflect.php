...
$branch_status = $_POST['branch_status'];
		...
		while($row_payment = mysql_fetch_assoc($sq_payment)){
			if($row_payment['amount']!=0){
				...
				$sq_pay = mysql_fetch_assoc(mysql_query("select sum(amount) as sum ,sum(credit_charges) as sumc from package_payment_master where clearance_status!='Cancelled' and booking_id='$row_payment[booking_id]'"));
				...
				if($row_payment['payment_mode']=="Cash" || $row_payment['payment_mode']=="Cheque"){
					$checshow = '<input type="checkbox" id="chk_receipt_'. $count .'" name="chk_receipt" data-amount="'. $row_payment['amount'] .'" data-payment-id="'. $row_payment['payment_id'] .'" value="'. $row_payment['payment_id'] .'">';
				} 
				...
				$temp_arr = array( "data" => array(
					...
					$payshow,
					number_format($row_payment['amount']+$row_payment['credit_charges'],2),
