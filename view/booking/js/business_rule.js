function get_auto_values(
	...
	if (charges_flag === 'true') {
		...
		//console.log.log(service_charge1 + ' Service charge');
		/////////////////Service charge End/////////////////
	...
	if (type === 'save') {
		calculate_total_discount(sub_total);
	}
	else {
		calculate_total_discount(sub_total);
	}
...
function get_tax_charges(
	...
	//console.log.log(service_charge + ' Service charge amount');
	if (type === 'save') {
		var service_tax_subtotal = 'txt_service_charge';
		...
		new_taxes_rules &&
			new_taxes_rules.map((rule) => {
				...
				if (target_amount === 'Service Charge') {
					...
					var charge_amount = parseFloat(basic_amount);
				}
				...
				console.log(new_service_charge + "new_service_charge")
				new_service_charge =
				...
				console.log(charge_amount + ' Charge_amt');
				console.log(tax_amount + ' tax_amt');
...
function get_tax_rules_on_conditions(final_taxes_rules, basic_amount, payment_mode, type) {
	let applied_rules = [];
	if (type === 'save') {
		var hotel_table = 'tbl_member_dynamic_row';
	...
	//console.log.log(final_taxes_rules);
	final_taxes_rules &&
						...
						condition.forEach((cond) => {
							...
							switch (condition) {
								case '1':
									...
									break;
								case '2':
									var booking_type = $('#tour_type_r').val();
								...
								case '5':
									...
									break;
								case '8':
									if (value == 'Group Tour' || value == 'All') flag = true;
									break;
								case '3':
									switch (for1) {
...
function get_charges_on_conditions(service_charge_result, basic_amount, payment_mode, type) {
	////console.log.log(service_charge_result);
	let rules_array =
		...
		service_charge_result.filter((rule) => {
			...
			// //console.log.log(rule.rule_id+'-'+conditions_flag_array);
...
function calculate_charges(rules_array, type, basic_amount, markup_amount1) {
	if (rules_array.length) {
		...
		//console.log.log(total_pax + 'TOtal PAx');
		switch (apply_on) {
			case '1':
				//Per pax
				total_pax =
