...
while($row_booking = mysql_fetch_assoc($sq_booking)){
    ...
    $emp_name = ($row_booking['emp_id'] != 0) ? $sq_emp['first_name'].' '.$sq_emp['last_name'] : 'Admin';
            ...
            $sq_payment_total = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum , (`credit_charges`) as sumc from hotel_booking_payment where booking_id='$row_booking[booking_id]' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
...
header('Content-Disposition: attachment;filename="HotelBooking('.date('d-m-Y H:i:s').').xls"');
