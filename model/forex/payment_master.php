...
public function payment_sms_notification_send($booking_id, $payment_amount, $payment_mode)
{
	...
	$message = "Dear ".$sq_customer_info['first_name']." ".$sq_customer_info['last_name'].", Acknowledge your payment of ".$payment_amount." ".$currency_code.", ".$payment_mode."  which we received for forex services installment.";
...
public function bank_cash_book_save($payment_id, $branch_admin_id)
{
	...
	$sq_booking_info = mysql_fetch_assoc(mysql_query("select customer_id,created_at,booking_type from forex_booking_master where booking_id='$booking_id'"));
...
public function payment_email_notification_send($booking_id, $payment_amount, $payment_mode, $payment_date)
{
   ...
   $sq_customer_info = mysql_fetch_assoc(mysql_query("select email_id,first_name from customer_master where customer_id='$sq_forex_info[customer_id]'"));
   $email_id = $encrypt_decrypt->fnDecrypt($sq_customer_info['email_id'], $secret_key);

	$due_date ='';
   $sq_total_amount = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from forex_booking_payment_master where booking_id='$booking_id' and clearance_status!='Cancelled'"));
   ...
   global $model;
   $model->generic_payment_mail('53',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $due_date,$email_id, $subject, $sq_customer_info['first_name']);
...
public function payment_update_email_notification_send($payment_id)
{
	...
	$sq_payment_info = mysql_fetch_assoc(mysql_query("select * from forex_booking_payment_master where payment_id='$payment_id' and clearance_status!='Cancelled'"));
...
public function whatsapp_send(){
   ...
   $booking_id = $_POST['booking_id'];
   $payment_amount = $_POST['payment_amount'];
   $sq_forex_info = mysql_fetch_assoc(mysql_query("select * from forex_booking_master where booking_id=".$_POST['booking_id']));
  
   $total_amount = $sq_forex_info['net_total'];
   $sq_pay = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from forex_booking_payment_master where clearance_status!='Cancelled' and booking_id=".$_POST['booking_id']));
...
$sq_customer = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id=".$sq_forex_info['customer_id']));
