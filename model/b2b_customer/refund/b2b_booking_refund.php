...
public function refund_save()
{
	...
	$sq_max = mysql_fetch_assoc(mysql_query("select max(refund_id) as max from b2b_booking_refund_master"));
	$refund_id = $sq_max['max'] + 1;

	$sq_payment = mysql_query("insert into b2b_booking_refund_master (refund_id, booking_id, financial_year_id, refund_date, refund_amount, refund_mode, bank_name, transaction_id, bank_id, clearance_status, created_at) values ('$refund_id', '$booking_id', '$financial_year_id', '$refund_date', '$refund_amount', '$refund_mode', '$bank_name', '$transaction_id', '$bank_id', '$clearance_status', '$created_at') ");
	...
	if($refund_mode == 'Credit Note'){
		...
		$sq_payment = mysql_query("insert into credit_note_master (id, financial_year_id, module_name, module_entry_id, customer_id, payment_amount,refund_id,created_at,branch_admin_id) values ('$id', '$financial_year_id', 'B2B Booking', '$booking_id', '$customer_id','$refund_amount','$refund_id','$refund_date','$branch_admin_id') ");
	}
	...
	if(!$sq_payment){
		...
		for($i=0; $i<sizeof($entry_id_arr); $i++){
			...
			$sq_max = mysql_fetch_assoc(mysql_query("select max(id) as max from b2b_booking_refund_entries"));
			$id= $sq_max['max'] + 1;;

			$sq_entry = mysql_query("insert into b2b_booking_refund_entries(id, refund_id, entry_id) values ('$id', '$refund_id', '$entry_id_arr[$i]')");
			if(!$sq_entry){
				$GLOBALS['flag'] = false;
...
public function refund_mail_send($booking_id,$refund_amount,$refund_date,$refund_mode,$transaction_id){
  ...
  $sq_b2b_info = mysql_fetch_assoc(mysql_query("select * from b2b_booking_master where booking_id='$booking_id'"));
  $cust_email = mysql_fetch_assoc(mysql_query("select * from customer_master where customer_id='$sq_b2b_info[customer_id]'"));
...
//   $sq_payment_total = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from hotel_booking_payment where booking_id='$booking_id' and clearance_status!='Pending' and clearance_status!='Cancelled'"));
	...
	$sale_amount =$sq_b2b_info['total_fee'];
  ...
  <table width="85%" cellspacing="0" cellpadding="5" style="color: #888888;border: 1px solid #888888;margin: 0px auto;margin-top:20px; min-width: 100%;" role="presentation">
    ...
    // }
          $content .= '</tr>';
  $subject = 'B2B Cancellation Refund ( '.get_b2b_booking_id($sq_sq_train_info['booking_id'],$year).' )';
  global $model;
