...
public function bank_cash_book_save($refund_id)
{
	...
	$particular = 'Payment through '.$refund_mode.' for '.get_hotel_booking_id($booking_id,$yr1).' for '.$sq_hte['no_of_nights'].' Night(s) stay from '.get_date_user($sq_hte['check_in']).' in '.$sq_hte['category'].' Room at '.$hotel_name .' * '.($sq_htm['adults']+$sq_htm['childrens']).' '.$cust_name;
