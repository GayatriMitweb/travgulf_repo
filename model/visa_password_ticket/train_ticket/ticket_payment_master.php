...
$finance_charges = number_format($finance_charges,2);
		...
		$payment_date = $payment_date;
		$payment_particular = get_sales_paid_particular(get_train_ticket_booking_payment_id($payment_id,$yr2), $payment_date1, $finance_charges, $customer_id, $payment_mode, get_train_ticket_booking_id($train_ticket_id,$yr1),$bank_id1,$transaction_id1);
		$ledger_particular = get_ledger_particular('By','Cash/Bank');
		$gl_id = 231;
		$payment_side = "Debit";
		$clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
		...
		$payment_date = $payment_date;
		$payment_particular = get_sales_paid_particular(get_train_ticket_booking_payment_id($payment_id,$yr2), $payment_date, $payment_amount1, $sq_passport_info['customer_id'], $payment_mode, get_train_ticket_booking_id($train_ticket_id,$yr1),$bank_id1,$transaction_id1);
		$ledger_particular = get_ledger_particular('By','Cash/Bank');
		$gl_id = $pay_gl;
		$payment_side = "Debit";
		$clearance_status = ($payment_mode=="Cheque" || $payment_mode == 'Credit Card') ? "Pending" : "";
	...
	$payment_amount = $payment_amount1;
	$payment_date = $payment_date;
	$payment_particular = get_sales_paid_particular(get_train_ticket_booking_payment_id($payment_id,$yr2), $payment_date, $payment_amount1, $sq_passport_info['customer_id'], $payment_mode, get_train_ticket_booking_id($train_ticket_id,$yr1),$bank_id,$transaction_id1);
	$ledger_particular = get_ledger_particular('By','Cash/Bank');
	$gl_id = $cust_gl;
	$payment_side = "Credit";
	$clearance_status = "";
...
public function bank_cash_book_save($payment_id, $branch_admin_id)
{
	...
	$sq_ticket_info = mysql_fetch_assoc(mysql_query("select customer_id,created_at from train_ticket_master where train_ticket_id='$train_ticket_id'"));
...
public function finance_update($sq_payment_info, $clearance_status1)
{
	...
	$sq_train_info = mysql_fetch_assoc(mysql_query("select customer_id,created_at from train_ticket_master where train_ticket_id='$train_ticket_id'"));
	...
	if($payment_amount1 != $payment_old_value){
		...
		if($payment_mode == 'Credit Card'){
			...
			$payment_date = $payment_date;
			$payment_particular = get_sales_paid_particular(get_train_ticket_booking_payment_id($payment_id,$yr2), $payment_date, $credit_charges_old, $customer_id, $payment_mode, get_train_ticket_booking_id($train_ticket_id,$yr1),$bank_id1,$transaction_id1);
			$ledger_particular = get_ledger_particular('By','Cash/Bank');
			$gl_id = $cust_gl;
			$payment_side = "Credit";
			$clearance_status = ($payment_mode=="Cheque"||$payment_mode=="Credit Card") ? "Pending" : "";
...
$finance_charges = number_format($finance_charges,2);
			...
			$payment_date = $payment_date;
			$payment_particular = get_sales_paid_particular(get_train_ticket_booking_payment_id($payment_id,$yr2), $payment_date, $payment_old_value, $sq_passport_info['customer_id'], $payment_mode, get_train_ticket_booking_id($train_ticket_id,$yr1),$bank_id1,$transaction_id1);
			$ledger_particular = get_ledger_particular('By','Cash/Bank');
			$gl_id = $pay_gl;
			$payment_side = "Credit";
			$clearance_status = ($payment_mode=="Cheque" || $payment_mode == 'Credit Card') ? "Pending" : "";
		...
		$payment_amount = $payment_old_value;
		$payment_date = $payment_date;
		$payment_particular = get_sales_paid_particular(get_train_ticket_booking_payment_id($payment_id,$yr2), $payment_date, $payment_old_value, $sq_passport_info['customer_id'], $payment_mode, get_train_ticket_booking_id($train_ticket_id,$yr1),$bank_id,$transaction_id1);
		$ledger_particular = get_ledger_particular('By','Cash/Bank');
		$gl_id = $cust_gl;
		$payment_side = "Debit";
		$clearance_status = "";
...
public function bank_cash_book_update($clearance_status)
{
	...
	$sq_ticket_info = mysql_fetch_assoc(mysql_query("select created_at,customer_id from train_ticket_master where train_ticket_id='$train_ticket_id'"));
	$booking_date = $sq_ticket_info['created_at'];
...
public function payment_email_notification_send($train_ticket_id, $payment_amount, $payment_mode, $payment_date)
{
   ...
   $date = $sq_train_ticket_info['created_at'];
	$yr = explode("-", $date);
	$year =$yr[0];
   $sq_customer_info = mysql_fetch_assoc(mysql_query("select email_id,first_name from customer_master where customer_id='$sq_train_ticket_info[customer_id]'"));
   ...
   global $model;
   $model->generic_payment_mail('49',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $due_date,$email_id,$subject, $sq_customer_info['first_name']);
}
...
public function payment_update_email_notification_send($payment_id){
	...
	global $model;
	   $model->generic_payment_mail('59',$payment_amount, $payment_mode, $total_amount, $paid_amount, $payment_date, $due_date,$email_id,$subject, $sq_customer_info['first_name']);
}
...
public function whatsapp_send(){
	...
	$payment_amount = $_POST['payment_amount'];
	$sq_ticket_info = mysql_fetch_assoc(mysql_query("select * from train_ticket_master where train_ticket_id=".$_POST['booking_id']));

	$total_amount = $sq_ticket_info['net_total'];
	$sq_pay = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from train_ticket_payment_master where clearance_status!='Cancelled' and train_ticket_id=".$_POST['booking_id']));
